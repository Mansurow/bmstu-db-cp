...
public class UserService : IUserService
{
    ...
    {
        return _userRepository.GetAllUsersAsync(Role.Administrator);
    }
    ...
    public async Task<User> GetUserByIdAsync(Guid userId)
    {
        try
        {
            var user = await _userRepository.GetUserByIdAsync(userId, Role.Administrator);
    ...
    public async Task ChangeUserPermissionsAsync(Guid userId, Role permissions)
    {
        try
        {
            ...
            await _userRepository.UpdateUserAsync(user, Role.Administrator);
        }
        catch (InvalidOperationException e)
        {
            _logger.LogError(e, "User with id: {UserId} not found", userId);
    ...
    public async Task CreateAdmin(string login, string password)
    {
        try
        {
            ...
            await _userRepository.InsertUserAsync(user, Role.Administrator);
