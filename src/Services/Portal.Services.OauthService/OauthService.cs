...
namespace Portal.Services.OauthService
{
    ...
    public class OauthService: IOauthService
    {
        ...
        public async Task Registrate(User user, string password) 
        {
            try
            {
                var userModel = await _userRepository.GetUserByEmailAsync(user.Email, Role.User);
            ...
            catch (InvalidOperationException)
            {
                ...
                await _userRepository.InsertUserAsync(user, Role.UnAuthorized);
            }
        ...
        public async Task<User> LogIn(string login, string password)
        {
            try
            {
                var user = await _userRepository.GetUserByEmailAsync(login, Role.User);
